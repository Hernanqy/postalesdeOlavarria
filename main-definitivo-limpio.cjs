...
function App() {
  ...
  async function startCamera() {
    if (!currentSpace) {
      setError("Escanea el QR del espacio para cargar una postal.");
      return;
    }
  ...
  function capturePhoto() {
    ...
    setTimeout(() => {
      ...
      if (!video || !canvas) {
        ...
        setError("No se pudo capturar la camara.");
        setStatus("idle");
        return;
      }
            ...
            {currentSpace ? (
              ...
              <div className="detectedCard">
                ...
                <div>
                  ...
                  <small>
                    Escanea el QR del museo o espacio cultural para comenzar.
                  </small>
                </div>
              </div>
            )}
...
fs.copyFileSync(path, path + ".bak-definitivo");
fs.writeFileSync(path, code, "utf8");

console.log("Listo: main.jsx reemplazado completo y limpio.");
